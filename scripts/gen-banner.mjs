...
function generateTypeScript(opts) {
  ...
  return [
    ...
    " *",
    " * @param log - 调用方（实例编排器或 cluster master）传入的实例级日志器；",
    " *   刻意不做模块级取值——banner 属启动期呈现，归属哪个实例由调用方决定。",
    " */",
    ...
    '  log.raw(process.env.NO_COLOR || !process.stdout.isTTY ? raw.replace(RE_ANSI_ESCAPE, "") : raw);',
    "}",
  ].join("\n");
}

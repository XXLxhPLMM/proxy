...
`utils/` 只允许依赖 `config/scope` 的**实例配置**（调用方显式传入 `ConfigScope`，只读取值键）
与其它 `utils/`；**禁止反向依赖 `server/`、`core/`**
（`config/` 内部的解析器如 `upstream-url.ts` 也不属于这里，见 `src/config/AGENTS.md`）。
**不得再出现模块级配置单例**（原 `config/store` 已删）：`log/level.ts` 收 `ConfigScope` 参数，
`net/upstream-tls.ts` 收它真正需要的两个值（`{ insecure, ca }`）以保持纯函数。
...
- 所有 `src/` 代码用日志器，不用 `console.*`（ESLint `no-console`，白名单只有 `log/logger.ts`）。**没有模块级 `logger` 单例**（已删，不留别名）：只有两条路——**实例级** `createInstanceLogger(scope, { prefix?, color? })`（库/组合根给每个实例各建一个，等级与落盘路径随实例走）或**进程级** `getLogger(prefix)`（只服务天然属于进程的日志：CLI 启动摘要、cluster master、协议层前缀）。`getLogger` 从 `setProcessScope(scope)` 登记的进程级 scope 惰性派生，**重复 `setProcessScope` 抛错、未登记就 `getLogger()` 也抛错**（绝不回退隐式默认 scope，否则多实例启动期日志全串到一份 LOG_LEVEL/LOG_FILE 上）。`Logger` 构造必填 `scope`（无无参构造），`child()` 透传 scope。
...
- console 与文件两道独立门限：值从 `Logger` 所属实例 scope 现取（`scope.get("logLevel")` console 默认 `error`、`scope.get("logFileLevel")` file 默认 `info`；两级回退都是「实例配置 > 终端 env > 缺省」），`LoggerOptions.level/fileLevel` 可为子模块定级强制覆盖；console 走人类可读文本（`<ISO> <LEVEL> <prefix> <msg> k=v`），文件走 **JSONL**（`log/YYYY-MM-DD-HH.jsonl`，每小时轮转）。直接写、无队列；每次 appendFile 登记进**该 Logger 实例自己的**在途集合（根 Logger 构造时新建、child 与父共享同一份，**跨实例绝不共享**——A 实例 `flush()` 不等 B 的写入），`await logger.flush()` 等齐本实例全部在途落盘后返回（`ProxyServer.stop()`/CLI 致命路径/cluster master 收尾在 `process.exit` 前调用；强退路径不等；正常事件循环退出无需）。`logger.raw()`（banner，不落盘）bypass 两道门；`logger.notice(level,…)`（生命周期/配置通知：启动摘要、worker 就绪、ACL·账号表热加载）绕控制台门限（`silent` 除外）、落盘仍按 `fileLevel`；notice 落盘走与常规行完全相同的 JSONL 管线（同一 schema/保留键/净化/小时轮转）。**刻意没有** `file()`/`both()`/`infoSync()`（零调用方）、`setLevel()`/`setFileLevel()`/`setFile()`（等级与落盘基址的真相源是所属实例 scope，`child()` 只透传不写入）与 `LoggerOptions.file` 落盘基址覆盖（覆盖即让日志器越出自己实例的文件；`level`/`fileLevel` 只压等级、不碰落盘路径，才可保留）——需要新通道时按职责新增并同时登记调用点，不要先留「预留」导出。调用永不抛：循环/BigInt/Symbol 有回退，两通道 try/catch。所有字符串参数做控制字符转义（线数据不得伪造日志行或注入终端转义）；日志目录/文件 `0o700`/`0o600`。
- **结构化字段**：`logger.info("msg", { ...fields })` —— 最后一个纯对象参数即字段（`text.ts:isPlainObject`（文件内私有）原型检查天然排除 `Error`/`Array`/`Buffer`/`Date`）。文件通道合并进记录顶层，console 渲染 `k=v`。保留键 `ts/level/pid/prefix/msg` 优先，同名字段被忽略。行形态：`{"ts":"2026-09-20T14:03:11.201Z","level":"info","pid":1234,"prefix":"[proxy]","msg":"[forward]","client":"1.2.3.4","target":"example.com:80","method":"GET","user":"alice"}`。查询：`jq -r 'select(.user=="alice") | .msg, .target' log/*.jsonl`；`jq -r 'select(.msg=="[auth] deny") | .client' log/*.jsonl | sort | uniq -c`；`jq 'select(.level=="warn")' log/*.jsonl`。
- **值渲染只有一份实现**：`text.ts:renderValue` 是 msg 参数与结构化字段共用的唯一入口（`renderFieldValue` 只多一层「null/undefined 跳过该键」）。**禁止再写第二份 stringify/字段渲染**——`NaN`/`Infinity` 因此保真为 `"NaN"`（不塌成 `JSON.stringify` 的 `null`），改这里等于改全部日志形态。`Error` 特判为可读单行 `name: message [code=...] [stack 首帧]`（`renderErrorText`，经 `sanitizeLogText` 净化；与 `isPlainObject` 同为**文件内私有**函数，不是导出 API）——`JSON.stringify(Error)` 只会得到 `{}`，转发层 502 的成因（ECONNREFUSED/TLS 校验失败）不能丢；控制台 msg 通道不变，Error 仍原样交给 `console.*`（原生堆栈可读）。fields 判定不受影响（Error 仍不是 fields）。
- **两种控制字符净化语义不可混用**：`sanitizeLogText`（转义为 `\n`/`\x1b` 等可见形式，保单行可读）与 `stripControlChars`（原地折叠为空格）。`file/json-error-text.ts` 的去敏错误文本用后者 + 空白折叠，**不得**改用前者（错误文本要的是「压平」不是「可读转义」）。
- **事件码**（`events.ts`）：`[ip-denied]` / `[target-denied]`（warn，带 `client`/`target`/`reason`）；`[tls-client-error]`（warn，TLS 握手失败含 mTLS 拒绝，带 `code`/`authorizationError`）；forward/auth 行带 `user`。事件码是**稳定 grep 契约**，重命名即 breaking change；同一语义只写一次格式，新增事件 = `LogEvent` 一行 + 工厂调用一行（`LogEvent` 本身是**文件内私有**的码表，对外只有 `log*` 工厂函数；调用方不要 import 码表自己拼字符串，否则 grep 契约就散了）。`EventLog` 因此含 `debug`；`makeEvent`/`makeExtraEvent` 的最后一个参数是 `LevelOverride`（只允许 `"debug"`，不得借它把事件改成 error，也不得借它删字段）。`events.ts` 是零依赖叶模块（自带最小 `EventLog` 接口），因此 `net/tls.ts`、`core/server/socks-base.ts`、`server/index.ts` 都能直接引它——**它曾是 `server/log/events-log.ts`，为了让 utils 不反向依赖 server 才下沉的**。
- `setupProcessGuards(log: Logger)`（`process/guards.ts`）捕获 `uncaughtException`/`unhandledRejection`/`warning`（只记不退出），由 `ProxyServer.start()` 传入**本实例**的 `Logger` 并取得一个幂等 lease/disposer；同一进程只安装一组物理 listener（打印出口锁定为**装上它的那次 lease** 的 logger，后到 lease 复用不覆盖——否则 A 实例崩了却查不到 A 的日志文件），最后一个 lease 释放时移除，避免重复日志与 `instance.stop()`（经 `ProxyServer.stop()` 归还 lease）后残留的宿主 handler；不另加 `uncaughtExceptionMonitor`。**刻意不用 `getLogger()`/`setProcessScope()` 进程级门面**：那套门面是给「天然属于进程而非某个实例」的日志用的，而未捕获异常在多实例库里说不清归属（宿主自己的 bug 也会落到这里），显式收 logger 形参即调用方的选择。安装回滚与 lease disposer 均逐项执行，某一个 `removeListener` 抛错不会跳过后续 handler；清理错误由 guard 记录并以聚合错误交给资源所有者，不能静默吞掉；失败时保留可重试的物理 listener 引用，下一 lease 先重试移除，避免下一 start 误判已绑定或重复安装。

## 网络与 TLS（`net/`）

- **`tls.ts` 是入站 mTLS 唯一入口**：`loadCerts` / `requiresClientCert` / `tlsServerOptions` / `bindTlsClientError` 供 https 与 TLS SOCKS 共用；`upstream-tls.ts` 是出站 TLS 唯一入口：`readUpstreamCa` / `upstreamTlsOptions`，`src/plugins/forwarders.ts`（传输策略组 `https.request` 选项）与 `core/forward/dial.ts:dialTls`（`tls.connect`）共用，两处都只吃 `ForwardPlan.upstreamTls` 冻结的策略。**入站失败 abort 启动、出站失败回退系统信任库**，两者语义相反，绝不互相引用或合并。
...
- **`upstreamCa` 默认空串 = 系统信任库**；一旦配置则**整体替换**系统库（只信任它），公网 CA 上游必然 `UNABLE_TO_VERIFY_LEAF_SIGNATURE` —— 串联公网 HTTPS 上游留空，自签上游才填。读取收敛在 `upstreamTlsOptions` 内的私有 `readUpstreamCa`（非普通文件返回 `undefined`，防 `readFileSync` 抛 EISDIR），不是对外入口。

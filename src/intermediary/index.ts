...
function sendProxy(
  req: typeof http.request | typeof https.request,
  Agent: typeof HttpProxyAgent | typeof HttpsProxyAgent,
  d: ProxyRequest,
): Promise<string> {
  return new Promise((res, rej) => {
    // 建立请求
    ...
    if (d.data) {
      ...
      if (
        ...
        d.headers["Content-Type"].includes("json")
      ) {
...
export function runIntermediary() {
  ...
  app.use(async (ctx, next) => {
    ...
    let Agent: typeof HttpProxyAgent | typeof HttpsProxyAgent | undefined =
      undefined;

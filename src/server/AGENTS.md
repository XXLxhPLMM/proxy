...
- `bindProxyEventLogs()`：core 只抛事件不直接打印（握手/接入期那条保留例外见 `src/core/AGENTS.md`），落盘收拢于此。**事件函数从 `@/core/log-events.js` 取**（`logUpstreamRefused`/`logLoopDetected`/`logBadRequest`/… 及 `LogEvent`；原先住在 `server/log/events-log.ts`，已下沉到 core，导出符号一字未改）——只有直接调用方是 `core/server/*`，留在本层会逼出 `core → server → core` 的目录环。强类型 `ProxyEventMap` 经本文件的 `ProxyEventSource` 端口（`asProxyEventSource`，core `ProxyCore` 刻意不暴露 EventEmitter）**直接 `on`/`off` 订阅**落盘，**不经 `EventHub` 中转**——旧实现把 `AppEventMap` 用 `as unknown as ProxyEventBus` 整个抹掉、只换来一次多余 publish 和 listener 快照拷贝，已删除（`ProxyEventBus`/`ProxyEventEnvelope`/`asProxyEventBus`/`logSubscriptions` 全部随之消失）。因 listener 直接跑在 Node EventEmitter 的 emit 栈上，**每个 listener 体内必须自带 `try/catch`**：handler 抛错会顺着 emit 反向打断 core 主流程（`ForwarderBase.emit`/`BaseProxy.emit`）并连带阻断同事件名下的其它 listener，退订动作收在 `eventDisposers`。消息文本、等级、字段、敏感头掩码与 JSONL 契约不变。转发 debug headers 会掩码 `proxy-authorization`/`authorization`/`cookie`；`auth` allow→debug、deny→info；`pipe: route` 与 `[route]` info 行 1:1；`upstream-error` 带 target/err.message 落 warn，保留 502 成因。

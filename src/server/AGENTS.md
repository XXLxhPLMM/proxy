...
- `bindProxyEventLogs()`：core/server 只抛不记，落盘收拢于此。`forward`（按 kind 打行）/ `forwardError` / `serverError` / `clientError` / `auth`（allow→debug，deny→info 留审计）/ `pipe`（按 `type` 分发：`target-unresolved/loop-detected/upstream-refused/upstream-error/upstream-timeout/route/ip-denied/target-denied/socks/debug`；**转发层 502 必须带成因**：`upstream-error` 含 `target` + `err.message` 落 warn，否则 TLS 失败/ECONNREFUSED 在 info/error 级无痕）。

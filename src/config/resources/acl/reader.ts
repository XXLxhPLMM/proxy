 ...
 *
 * 职责：把「调用方传入的路径」变成「该实例当前生效的 ACL 配置」，并把读取层的
 * 状态迁移桥到资源事件总线。结构校验在 `schema.ts`，判定在 `eval.ts`，日志呈现由
 * `resources/notice.ts` 统一订阅，本文件不得直接写日志。
 *
 * 语义：文件缺失 = 全部放行（fail-open，绝不改成 fail-closed）；内容非法 = 保留
 * 上一份有效值。路径必填，由调用方从自己的配置作用域取出——本读取器不回读任何
 * 进程级单例，同进程多个实例因此各自持有互不可见的名单。
 */
...
/**
 * 读取 acl 文件（带节流缓存）
 * @param opts.path - 名单文件路径；由调用方从自己的配置作用域取出（必填）
 * @param opts.force - 跳过节流强制重读（启动期校验用）
 * @returns 读取结果：value 为生效配置，error 为最近一次失败原因
 */
export function readAcl(opts: { path: string; force?: boolean }): JsonFileRead<AclConfig> {
  return readJsonCached(opts.path, validateAcl, {
    label: "访问控制名单文件",
    fallback: EMPTY_ACL,
    resource: "acl",
    force: opts.force,

...
- **访问控制判定不在本目录**：名单数据在 `files/acl.ts`，**条目规则**（解析/编译/匹配，即「什么算合法条目、怎么命中」）在 `files/rules/`，请求期判定在 `src/core/access-control.ts` —— 那里是 `AccessControl` 端口的**唯一**内置实现 `createFileAccessControl(config)`，三个判定方法 `checkClient({ client })` / `checkTarget({ host, user? })` / `checkRoute({ host })` 收在它返回的对象上（三个判定函数本身是**模块私有、不导出**的）。黑白名单优先级、整组缺失回退、命中后拒绝还是直连都在那里。**`config` 在闭包之外拿不到 `config` 形参**：三个方法都没有 `config` 形参，工厂捕获一次——**同一件事只留一个传法**，开第二个入口迟早出现「拿 B 实例的名单、判 A 实例的请求」（症状是「名单时灵时不灵」）。改名单**语义/动作**动 core，改**文件格式与条目语法**动 `files/rules/`（连带 `files/acl.ts` 的形状校验）。
...
- `users.ts`：`AuthAccount`（含可选 `acl?: UserPolicy` 与可选 `quota?: UserQuota`）+ `UserPolicy` / `UserPolicyList` / `UserQuota` + `validateAuthUsers`（形状校验）+ `readAuthUsersAsync`（启动期 **fail-closed** 强校验，绕过热加载缓存与事件；**只把 `ENOENT` 当缺失**） + `readAuthUsers` / `loadAuthUsers` / `loadUserPolicy` / `loadUserQuota`（同步热加载面，`opts.config` 必填）。
  ...
  - **`quota.window` 只认 `day` / `month` 两个日历窗**，缺省 = `month`。其它取值（`"week"` / `"hour"` / `"rolling"` / 大小写变体 / 非字符串 / `null`）→ **整组非法 → 启动期 abort**：收下「配了、实际按 month 跑」的值等于给假的安全感。**为什么不做滚动窗**（`"30 天内 100GB"`）：① 解释成本——运维看到「已用 98GB/100GB」时答不出「为什么现在被拒」，而配额是**要被运维解释**的东西；② 聚合成本——滚动窗不能只存一个标量，判定要跨多个历史窗口求和，与账本「**滚动即清账**」的惰性模型（无定时器、无后台任务）不相容；③ 本项目处于设计期，**不预留占位值**——塞一个 `window:"rolling"` 却按日历窗跑恰是「配了但没生效」的最坏形态。真要支持必须连同账本形态（滑窗队列 + 落盘格式）一起设计。
...
- `acl.ts`：`AclList` / `AclConfig` + `validateAcl` + `readAclAsync` + `readAcl` / `loadAcl` / **`hasConfiguredAcl`**。**只管拿数据与形状校验，不做判定**；条目合法性靠同目录 `rules/` 的 `parseIpRule` / `parseHostRule` 判，任一条非法 → 整组编译失败 → 启动期 abort。
  - **`hasConfiguredAcl(config, onFileEvent?)`：「配了访问控制」的**文件事实**判定**（三组名单任一非空即 `true`）。它与 `runtime/services.ts:hasConfiguredQuota` **完全同构**，存在的理由也相同：它是 runtime 启动期 `acl-inert` 告警的判据（「注入了自定义 `access` → `acl.json` 不会生效」），而**告警与判定必须是同一个函数**——两处各写一份，迟早出现「告警说没配、判定说配了」。**签名刻意只收 `ConfigAccessor`**（与 `hasConfiguredQuota` 同款理由：它是一份纯判定、不观察也不发布，`logger`/`events` 对它纯多余）。经 `@/config/index.js` 出面、再由 `runtime/services.ts` **转出**（那条转出与 `hasConfiguredQuota` 住在同一层，让 `runtime.ts` 从一处取两个启动期判据）。
  - **⚠️ 复用既有读取路径，绝不许另开一个 `readJsonCached` 调用点**：走的就是 `loadAcl` → `readJsonCached`（缓存键 `label + path`、1s stat 节流、坏内容保留上一份有效值）。另开会造成**两份节流缓存、两份解析、两套坏文件处理**并互相污染同一缓存键——`users.ts` 那条同类纪律**有变异测试**（给 `loadUserQuota` 另开读取器 → 源码断言 + 「一次内容变更只报一次 `reloaded`」同时红）。护栏 `tests/unit/acl-configured.test.ts` 逐条断言：`acl.ts` 全文 `readJsonCached(` **恰好一处**、判据本体只经 `loadAcl(` 且零 `fs.`。
  - **⚠️「读失败 → false」是刻意取舍，不是遗漏**（取舍要写在这里，否则下一个人会当 bug 修）：整份缺失 / 四组全空 / **读失败**（`EACCES` 等 stat 错误、坏 JSON 且无历史）一律 `false`，于是**读不到名单时不告警**。语义是「**压根不知道配没配**」，不是「配了却没生效」——报出来是**误报**。**宁可少告警也不误报**：一条会误报的告警在第一次误报之后就再也不会被看，那等于把这条告警永久关掉。**代价的兜底是「必须有别的可见信号」**：读失败时 `readJsonCached` 经 `onEvent` 报 `error` → runtime 发 `config.file-error` → CLI 落日志。护栏**同时**断言这两件事（判据 `false` **且**恰好一条 `error` 事件），否则「读失败静默 false」就是货真价实的假阴性。
  - **三组逐个列举而不是遍历键**（`acl.clientIp.whitelist.length > 0 || …`）：判据要**读得出每组各自是什么**；遍历 `Object.values` 在「将来加了第四组」时会静默把新组算进去，而那需要一次**显式的语义裁决**（新组算不算「配了访问控制」？它的缺席是不是也是取消防护？）。`upstream` **算在内**——它是 client 模式的**路由名单**，同样写在 `acl.json` 里、同样会因 `access` 被覆盖而不生效（后果不同：该回落直连的仍走上游）。
...
这两个文件是 `acl.json` 的**条目语法层**，住 config 而不是 utils 的理由：它不是通用网络基础设施——通用工具目录不该知道「名单条目」这个业务概念（`@/utils` 侧只留零业务概念的 `@/utils/host-text.ts` 文本原子）。`ipToString`（字节 → 文本，供审计/诊断渲染）也在本层导出。
...
3. CLI 用 `defaultEnvFileNames(env.NODE_ENV)` 显式生成低→高候选：原始顺序固定为 `.env.production` → `.env.development` → `.env.<NODE_ENV>`，后者覆盖前者；重复名只保留最后一次。因此 `NODE_ENV=production` 去重后的实际读取顺序是 `.env.development` → `.env.production`（production 仍胜出）。该函数只产名字，不扫描也不读取文件。`start/start:dev/start:prod` 仅设置 `NODE_ENV`，**不得**用 Node `--env-file` 预注入。
4. `loadConfig` 返回 context 后，CLI 严格执行 `createLogger({ config: context.accessor })` → 输出 `context.warnings` → `runServer(context, { logger, noColor: Boolean(env.NO_COLOR), trafficWorkerSlot: normalizeSlot(env[TRAFFIC_SLOT_ENV]), assembly: cliPreset() })`。⚠️ **`runServer` 只接一个 `RunServerOptions`**（**不接受位置参数**；它还带 `processPolicy` / `services` / `connectors` / `assembly`）。`trafficWorkerSlot` 是流量配额账本槽位号，只从上面那份 env 快照取（细节见 `src/core/AGENTS.md` 的 traffic 一节）；`assembly: cliPreset()` 是「CLI 就是库预设的一次组装」这句话的代码落点。
...
| `QUOTA_LEDGER_DIR`                                                                                                       | **startup** 配额账本目录，默认 `<configDir>/cfg/quota`（标 `path: true`，相对值按 configDir 绝对化）。**必须是 startup**：运行中改目录 = 已打开的 append 句柄仍指向旧文件，改了等于没改；改它要重建 runtime。**被 `core/traffic/ledger.ts` 消费**：`<dir>/worker-<slot>.jsonl`，`slot` 是稳定序号（单进程 `"0"`，cluster worker `1..N`，由 `server/cluster.ts` fork 时注入 `PROXY_WORKER_SLOT`）。**没有用户配非全 0 `quota` 时这个目录不会被创建**（零成本档） |
...
| `TLS_CA`                                                                                                                | client-cert CA = **mTLS switch**. Empty (default) = server-only TLS; set = client certs **required** on `https`/`sockss4`/`sockss5`, unreadable file aborts startup. No default file（机制见 `src/utils/AGENTS.md` 的 TLS 一节，实现在 `utils/tls/`）；**它标 `path: true`** |
...
| `QUOTA_FLUSH_INTERVAL`                                                                                                  | runtime delta 落盘间隔 ms，`int min 1`，默认 `5000`。**被 `core/traffic/flush-loop.ts` 消费**：自重排 `setTimeout`（间隔每次现读 → 热改在下一轮生效）、`unref()`。`0`/负数**启动期 abort**（`0` 不等于「关掉落盘」）；**停机落盘与这个间隔无关**（`runtime.stop()` 必做最后一次 flush），所以「间隔配很大 + 反复 Ctrl+C」刷不出额外额度 |
...
- **流量配额的三个运行参数**（`quota` 本身住在 `users.json`，这三个是围绕它的）：`quotaLedgerDir`（**startup** + `path: true`，见 env 表理由）、`quotaResetHour`（runtime，`int {min:0,max:23}`）、`quotaFlushInterval`（runtime，`int {min:1}`）。**分流的事件面已被护栏锁住**（`tests/unit/quota-config-fields.test.ts` 断言 `keysByPhase()` 的归属，`tests/unit/proxy-runtime.test.ts` 的「流量配额三个配置项按相位分流」经**真 runtime** 断言 `config.restart-required` vs `config.changed`）——分类写错的后果分两种：账本目录被标 runtime 会「看起来生效」而实际句柄没换；`resetHour` 被标 startup 则热改必须重启，运维会以为配置坏了。
- **三行都有真实消费方**：`quotaLedgerDir` → `core/traffic/ledger.ts:JsonlTrafficLedger`（`runtime/services.ts:buildDefaultServices` 注入，**目录只在有非全 0 配额时创建**）；`quotaResetHour` → 同一个账本的窗口口径（**与判定共用同一 accessor 闭包**，所以恢复与判定不可能算出两个窗口）；`quotaFlushInterval` → `core/traffic/flush-loop.ts:startFlushLoop`（唯一定时器站点，`unref`）。**新增配置项时先确认有消费方，否则它就只是一行文档**。
...
- 被拒各打一条 warn：`[ip-denied]`（带 `client`/`reason`）或 `[target-denied]`（带 `target`/`host`/`reason`/`user`，**文本带 ` source=<global|user>`**；判定层没给 `source` 时靠条件拼接而非无条件加键）。
- 判定入口全部收在 `src/core/access-control.ts:createFileAccessControl(config)`（`AccessControl` 端口的**唯一**内置实现；三个判定函数本身模块私有、不导出，调用点拿不到裸函数）。三行入口：`services.access.checkClient({ client })`（`core/server/http.ts:handleForward()` 最先、`socks-base.ts:onConn()` 首行，均早于鉴权；`client` 是 TCP 对端地址）、`access.checkTarget({ host, user })`（四条转发路径 http/tunnel/websocket/socks，均在目标已解析、尚未拨号处，紧邻现有 `isSelfLoop` 守卫；`user` 由 `ForwarderBase.preDial` 从 `scope.user` 注入，而 `preDial` 是全仓唯一读 `scope.user` 的地方，链是 `RequestScope.user → preDial → guardPreDial({ access }) → access.checkTarget`）、`access.checkRoute({ host })`（仅 `core/helpers/route.ts:resolveRoute` 在 `mode !== "server"` 时调用，client 模式路由判定）。**判定顺序**：clientIp → auth → target ACL（403，永不旁路）→ 路由判定 → 拨号。**判定对象永远是「客户端请求的目标」**：absolute-form 取 request-target 的 authority（RFC 7230 §5.4），缺失时回退 `Host`；**与 `proxyMode` 无关**——client 模式下拨号目标是上游，而上游的协议/地址/端口只来自 `UPSTREAM_*`、**永不进名单**。
  - **⚠️ 三个方法全同步是硬裁决，不是省事**：`checkRoute` 被四条入站通道在**拨号之前**调用，它的返回值要立刻喂给「选连接器 / 拒绝应答 / 发 `route` 事件」这一串**同步**控制流。改成 `async` 会级联改掉整条转发链（每条通道各多一个 `await` 边界、`resolveForwardTargets` 连带变 async、转发器入口签名与两阶段准入的时序全部要重排），为「将来也许要查个远程策略」付这个代价不划算。**需要远程查策略的诉求归 `identity` 不归 `access`**——身份判定本来就是 `async` 的（「身份判定的结果允许等，准入判定不允许」）。护栏在 `tests/unit/access-control-port.test.ts`：断言「返回值不是 thenable」而不是「函数不是 async」（后者会被「async 但内部同步返回」骗过）。
  - **⚠️ `proxyMode` 模式门在 `resolveRoute`、不在判定层**：`checkRoute` 是纯名单判定、不读 `proxyMode`；`resolveRoute(dest, { access, mode })` 把 `mode` 当**配置事实**与 `access` 端口**并列**（两个形参，不合成一个「什么都能读」的访问器——那才是第二真相源），并在**第一行**对 `mode === "server"` 短路、不问 `access`。为什么不塞进判定层：`proxyMode` 是**路由模式**决定，与授权正交；塞进去会迫使每个自定义策略实现重写一遍模式门，且 `checkRoute` 的硬不变量（不带模式限定的真值表）会长出一个模式限定。丢了短路的**具体可观测后果**：`forward/base:emitRoute` 的跳过条件是 `mode === "server" && !reason`，于是 `proxyMode=server` 且配了 `upstream` 组的部署会**凭空多发一条 `route` 事件 / 多落一行 `[route]` 日志**。
  - 回归护栏见 `tests/integration/client-mode-acl.test.ts`。
- **每用户名单（`users.json` 的 `acl.target`）与全局 target 组合流**：`放行 ⇔ 全局 target 组放行 ∧ 该用户 target 组放行`。
  ...
  - 个人组与全局 target 组**语义完全同形**（黑名单命中 → 拒；白名单非空且未命中 → 拒；皆空 → 放行），且两层在 core 里**共用同一个判定函数**（`hostDenied`），不存在第二套语义。
  - 用户未配 `acl`（或用户不存在 / 未鉴权）→ 个人层**中性放行**，等价于只有全局生效。
  - 个人名单**绝不**参与 `checkClient`（判定发生在鉴权之前，那时没有身份——fail-closed 的理由）与 `checkRoute`（client 模式的路由决策，与「你是谁」正交）；这两个函数体里**连 `user` 三个字母都不许出现**，源码级护栏按函数体断言。
  - 拒绝时 `AccessDecision.source` 标明是哪一层（`"global"` / `"user"`）。落盘 `[target-denied]` 行会多出 `source=…`（`user` 字段本来就在），运维据此知道该改 `acl.json` 还是 `users.json`。
  - **⚠️ `reason` 从闭合集放宽为 `string`（端口化）带来的影响**：端口对外是自由文本（替换实现可能是限速引擎 / 地理封锁 / 订阅制网关，闭合集会让它们**没法用类型描述自己的结论**，只能回去 `as never` 强转），但代价是**消费方不能再假设取值**。原「`reason` 只认 `{whitelist, blacklist}` 闭合集、表外值静默不发布事件」那条 `runtime/bridge.ts:aclReason` 收窄逻辑**已撤**，改成 `passthroughReason`（**非空即逐字透传**，只把「缺失/空串」判为跳过）——自定义引擎的 `"user:blacklist"` / `source: "geoip"` 会原样到达事件面。因此「内置引擎的 `reason` 只有两个字面量、`source` 只有两个」从**编译期保证降级为源码级自律 + 运行期断言**（护栏在 `tests/unit/user-acl-merge.test.ts` 与 `tests/unit/access-control-port.test.ts`）。**两条纪律没有随之消失，只是落点从消费者搬回生产者**：① **分层信息不许塞进 `reason`**（`hostDenied` 只返回两个字面量、`source:` 字面量集合恰为 `{global,user}`）；② **缺失/空串即跳过、绝不倒填成 `global`**（倒填会把「个人名单拒的」伪装成「全局拒的」，运维去改错文件）。
  - 护栏：`tests/unit/user-acl-merge.test.ts`（3×3 优先级真值表穷举、内置引擎 reason 集合、无身份即无个人层、不越界、热加载、零分配、事件 source 透传）+ `tests/integration/user-acl-enforcement.test.ts`（四条路径各一条「被个人名单拒」+ 恰好一条事件、全局优先、两次 preDial 不重复发事件、不重启即生效）。
...
| 场景             | 入口                                                                                              | 落点                                       | 宿主副作用                                                                                          |
| ---------------- | ------------------------------------------------------------------------------------------------- | ------------------------------------------ | --------------------------------------------------------------------------------------------------- |
| CLI / 自建可执行 | `cli.main()` 快照宿主来源 → `loadConfig` → `createLogger` → `runServer(context, { logger, noColor, trafficWorkerSlot, assembly: cliPreset() })` | 本进程独占 `ConfigStore` + `ConfigContext` | 仅 CLI 组合根读取 env/argv/cwd/NO_COLOR；加载器不写 `process.env`；server 层才安装守卫/信号/cluster |
...
- **无第二套 argv 解析入口**：argv 只有 `sources/argv.ts:parseRawArgv` 一处实现，`loadConfig` 是唯一把它变成配置的入口；argv 归一与字段解析的回归护栏一律经 `loadConfig` 断言。
- **`ConfigStore` 零 IO**：值从哪来永远由构造参数或 `loadConfig` 决定；实例化不执行 FIELDS 解析、范围、文件或 auth 交叉校验。
- **名单原语只有一份，住在 `files/rules/`**：`@/utils` 侧只剩文本原子 `@/utils/host-text.ts`。自环判定（`core/helpers/self-loop.ts`）、SOCKS 出站格式化（`core/forward/channel/socks.ts`）、拨号前归一（`core/forward/upstream/dial.ts`）都从 `@/config/files/rules/index.js` 取 `normalizeIp`/`normalizeHost`/`ipToString`/`ipv6BytesToString`——**core 依赖 config 的规则层是既定方向**，别为了「utils 才是底层」把它搬回去，那会重新引入「通用工具知道 acl.json 业务概念」的问题。护栏见 `tests/unit/acl-rule-ip.test.ts` / `acl-rule-host.test.ts`（断言不得改，只许改 import 路径）。
...
- `proxyMode` `server` vs `client` 决定 `resolveRoute` 的有效模式（server 读 URL/Host 直拨；client 拨 `upstreamHost`/`upstreamPort`，但 `upstream` 路由名单命中即回落直拨真实目标）——见 `src/core/AGENTS.md`。⚠️ 注意模式门归 `resolveRoute`、**不在** `AccessControl.checkRoute` 里（`checkRoute` 是纯名单判定），详见上面「访问控制（ACL）」那节的两条 ⚠️。
- **env 的影响全部收敛在 `loadConfig`；库层零 `process.env`**。`createProxyRuntime` / `runtime/presets.ts:pickStartupPreset` / 连接器 registry 都不读宿主 env——再读一次就是「协议由两处决定」的第二真相源（容器里 `PROXY_PROTOCOL=socks5` 起服务、库层从 env 读回另一个值 ⇒ 配置里写的协议与实际跑的协议不一致，且**没有任何日志或事件**能解释这个差异）。选协议服务器用 `PROXY_PROTOCOL`（它本来就是这个职责的 env 键）；要**具名装配**用 `StartupPreset` + `createProxyRuntime({ assembly })`，那份决策写在代码里。**刻意不新增 `STARTUP_PRESET` 配置键**：为它动 `FIELDS` + `defaults` + `.env.example` + `setup-env.ts` + 两条护栏共六个文件不划算，而 `PROXY_PROTOCOL` 已覆盖「用哪个协议服务器」。护栏：`tests/unit/startup-preset.test.ts`（`pickStartupPreset` 与 `runtime.ts` 的源码级零 `process.env`/`argv`）。
- 无全局回归护栏：`tests/unit/config-store.test.ts` 断言模块不导出 `defaultConfigStore`；`tests/unit/config-access.test.ts` 断言两个 accessor 隔离、热改现读、`ProxyOptions.ctx`/`createIdentityFromConfig(ctx)`/`createFileAccessControl(config)`/`resolveRoute(dest, { access, mode })` 必须显式注入（身份工厂第一参是 `CoreContext`；`resolveRoute` 收 `RoutePolicy = { access, mode }`，两者并列不合并）；`tests/unit/config-instance.test.ts` 锁定 context/accessor 隔离与快照冻结；`tests/unit/library-entry.test.ts` 与 `tests/library/entry.test.ts` 断言包入口没有 `get/getAll/set/globalConfigAccessor`，后者另有一条机器可读的「**一个旧名都不许再出现在包入口**」断言（含 `as` 别名绕过防护）。

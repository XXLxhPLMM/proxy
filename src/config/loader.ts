 ...
 * 设计：表驱动（FIELDS 描述全部字段），CLI 解析、env 合并、store 写入、快照返回
...
/** 按 keys 顺序取首个命中值（CLI 解析结果与 process.env 共用同一别名表） */
...
/** 有限数值（空串/NaN/Infinity 视为非法，回退默认）；范围约束（如 port 1-65535）由 zod 最终校验 */
...
const parseBool = (fallback: boolean) => (v: string): boolean => toBoolean(v, fallback);
/** 枚举：大小写不敏感白名单 */
const parseEnum = <T extends string>(values: readonly T[]) => (v: string): T | undefined => {
...
interface FieldDef<K extends ConfigKey = ConfigKey> {
  ...
  key: K;
  /** 环境变量名列表（首个命中生效）；CLI 别名同源，--key-name / KEY=VALUE 归一为 KEY_NAME */
  aliases: string[];
...
function loadEnvFiles(useHome: boolean): void {
  ...
  const candidates = [".env.production", ".env.development", `.env.${process.env.NODE_ENV ?? "development"}`];
...
export function parseStartupArgs(argv: string[] = process.argv.slice(2)): Partial<AppConfig> {
  ...
  for (const d of FIELDS) {
    ...
    if (parsed !== undefined) out[d.key] = parsed;
  }
...
export function initConfig(): AppConfig {
  ...
  const homeRaw = pickFirst(rawCli, HOME_CONFIG_ALIASES) ?? pickFirst(process.env, HOME_CONFIG_ALIASES);
  ...
  for (const d of FIELDS) config.set(d.key, resolved[d.key] as AppConfig[ConfigKey]);

...
- **映射契约**（4 条，无其它）：`auth` → `auth.decided`（`{passed,user,attempted,reason}`，身份维度进 context）；`pipe: ip-denied` → `access.client-denied`（`{client,reason}`）；`pipe: target-denied` → `access.target-denied`（`{host,target,reason}`）；`pipe: route` → `route.selected`（`{mode,route,reason?}`）。

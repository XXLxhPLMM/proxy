/**
 * SOCKSS4 加密 - tls (SOCKS4 over TLS)
 */
...
export class Sockss4Proxy extends BaseProxy {
  ...
  constructor(options: ProxyOptions = {}) {
    super("sockss4", options);
  }

  async onBeforeStart(): Promise<void> {
    if (!this.options.isWorker) this.log.info(`[lifecycle] sockss4 loading certs key=${this.options.tls?.key} cert=${this.options.tls?.cert} ca=${this.options.tls?.ca}`);
    this.certs = loadCerts(this.options.tls, this.log, "SOCKSS4");
  }

  protected async doStart(): Promise<void> {
    if (!this.certs) this.certs = loadCerts(this.options.tls, this.log, "SOCKSS4");

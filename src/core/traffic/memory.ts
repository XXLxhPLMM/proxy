 ...
 *
 * ## 窗口化：每用户槽位带一个窗口键，**滚动即清账**（Phase 5b-1）
 *
 * 5a 留下的账本形如 `Map<user, {up, down}>`，用量**只增不减**——那是刻意的（窗口语义
 * 当时还没定，先把「真用量」这件事做对）。5b-1 给每个槽位加一个 `windowKey`：进入
 ...
 *
 * **Phase 5b-2 解决了这条限制的「持久」那一半，并如实记录它没解决的那一半**：
 ...
 *
 * ## 落盘注入口（Phase 5b-2）：`consume` 仍同步，IO 全在账本侧
 *

...
export function handleSocks4(
  clientSocket: Duplex,
  initial: Buffer,
  ctx: { dial: (s: Duplex, h: string, p: number, head: Buffer) => void; log: { info: (...a: unknown[]) => void; warn: (...a: unknown[]) => void } },
): void {
  ...
  const needAuth = !!(get("authEnabled") as boolean) && (get("authType") as string) !== "none";
...
export function dialSocks4(clientSocket: Duplex, host: string, port: number, head: Buffer, log: { info: (...a: unknown[]) => void; warn: (...a: unknown[]) => void }): void {

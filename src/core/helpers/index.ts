 ...
 *
 * 依赖方向（无环）：`credentials` / `target` / `self-loop` 是叶子；`wire → target`；
 * `headers → credentials`；`upstream → credentials`；`route → target`；
 * `predial → self-loop`。
 *
 * 本文件**重导出全部公共面**（与拆分前的 `core/proxy-helpers.ts` 逐符号一致），
 * 层内实现（`indexMemo` / `splitAuthority` / `canonicalHost` / `WILDCARD_HOSTS` /
 * `LOOPBACK_HOSTS` / `MIN_PORT` / `MAX_PORT`）刻意不从这里出去。
 */

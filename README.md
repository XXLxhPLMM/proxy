...
- `target` 判的永远是**客户端请求的目标**，两种模式一致：absolute-form 请求取 request-target 里的权威值，否则回退 `Host`。**上游地址不受名单约束**——client 模式下前置代理拨的是 `UPSTREAM_*` 指定的地址，把上游写进 `target` 黑名单（或白名单里不写它）都不会拦住你自己的串联；上游只受自环守卫。所以白名单里只写「允许访问的网站」即可，不需要把上游地址也列进去。

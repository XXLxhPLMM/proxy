/**
 * 裸 node 自检脚本 - http 代理专用（明文外层）
 * 直连本机真服务 127.0.0.1:3000，日志直接走控制台 + log/ 文件
...
function testHttp() {
  return new Promise((resolve) => {
    ...
    s.on("end", () => {
      ...
      console.log(`[http] ${ok ? "PASS" : "FAIL"} - ${data.split("\r\n")[0]}`);
      resolve(ok);
    });
    s.on("error", (e) => {
...
function testHttps() {
  return new Promise((resolve) => {
    ...
    s.once("data", (d) => {
      const r = d.toString();
      const line = r.split("\r\n")[0];
      console.log(`[https CONNECT] ${line}`);
...
function testWs() {
  return new Promise((resolve) => {
    ...
    s.once("data", (d) => {
      const r = d.toString();
      console.log(`[wss CONNECT] ${r.split("\r\n")[0]}`);
      if (!r.includes("200 Connection Established")) {
...
async function main() {
  console.log(`=== proxy http://${PROXY_HOST}:${PROXY_PORT} auth=${AUTH} ===`);
  console.log(`提示：此脚本仅适用于 PROXY_PROTOCOL=http，明文连代理`);
  console.log(`      https 服务请用 node tests/manual/proxy-node-test-https.mjs`);
  ...
  console.log("\n=== SUMMARY ===");
  console.log(`http:  ${a ? "PASS" : "FAIL"}`);
  console.log(`https: ${b ? "PASS" : "FAIL"}`);
  console.log(`wss:   ${c ? "PASS" : "FAIL"}`);

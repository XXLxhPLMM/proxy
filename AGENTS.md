...
- **Env aliases** (loader.ts handles all, first-match wins):
  ...
  - `UPSTREAM_URL`: `REMOTE_URL` — 标准上游 URL `scheme://[user:pass@]host[:port]`（scheme ∈ http/https/socks5/tls），`utils/upstream-url.ts:parseUpstreamUrl` 严格校验（坏 scheme/空 host/path-query/端口越界 → env 抛错阻止启动），`applyUpstreamUrl` 拆项整体覆盖 `upstreamProtocol/Secure/Host/Port/Username/Password`（缺省端口按 scheme: http:80 / https,tls:443 / socks5:1080）；未配时 granular 拆项照旧生效

...
- **Core layer**: `src/core/types/` (`proxy.ts`: ProxyProtocol/ProxyOptions/ProxyStats/LifecycleState/ProxyCore/ProxyHttpServer/events; `auth.ts`: AuthContext/TokenExtractor/AuthProvider/AuthOptions leaf module; `server.ts`: http-server handler/options types; `pipe.ts`: PipeEvent/PipeEventSink; `index.ts` barrel) → `src/core/base.ts` (BaseProxy: pure lifecycle state machine + `authorize`, no server field; DirectServerProxy adds bare-server hold + `startListening`/`stopServer`/`attachErrorHandlers` for tls/socks; HttpProxy chain manages lifecycle via HttpServer/HttpsServer wrappers instead) + `src/core/http-server.ts` (single-file HTTP(S) transport: local bindEvents/startServer/closeServer helpers + tiny HttpTransport parent (fields/hooks/getters/start/close, private bindEvents) with flat HttpServer/HttpsServer children differing only in bare-server build; zero-log, no connection tracking/timeout knobs; Https loads certs via `loadTlsContext` without logger and throws enriched Error, logged by HttpsProxy.doStart) + `src/core/http-pipe.ts` (forwardHttp/forwardTunnel/forwardUpgrade pure functions, zero-log with optional PipeEventSink slot re-emitted as proxy "pipe" events; `proxyMode` switches target: server resolves from request URL/Host, client sends to `upstreamHost`/`upstreamPort` keeping absolute-form, `Proxy-Authorization` only from explicit `upstreamUsername/Password` (client headers never forwarded); client CONNECT has two branches — transparent by default (rebuilds CONNECT from method/url/rawHeaders minus proxy-* headers and pipes, upstream replies 200/407 directly; front auth already enforced beforehand in `authorizeAndForwardTunnel`) vs re-issue only when `upstreamUsername` is set (via `connectors/tunnel.dialTunnelViaUpstream`: dial+CONNECT+等200 resolve socket，非200 relay后reject)) → `src/core/connectors/` (`http/https/tls`: Promise<DialResult> 拨号，建链成功resolve socket，失败/超时reject；`tunnel.ts`: CONNECT下沉层，`sendConnect=false`=纯TCP直拨，`true`=发CONNECT+200才resolve；`socks.ts`: 占位reject；`http-pipe.dialUpstream` 复用 `dialHttpUpstream`) → `src/core/auth.ts` (Auth class + `extractUserFromToken` audit helper; token extraction lives in `src/core/token-extractors.ts`: header-only `HeaderTokenExtractor` (Proxy-Authorization/Authorization, RFC 7235); shared contracts `AuthContext`/`TokenExtractor`/`AuthProvider`/`AuthOptions` live in leaf module `src/core/types/auth.ts` to avoid an auth↔extractors cycle) + `src/core/connectors/` (upstream dialers, one file per protocol: http/https/socks/tls + index.createConnector factory; shared contracts UpstreamTarget/DialHandle/DialCallback/ConnectorDial live in leaf module `src/core/types/connector.ts`; http=wrapped net.connect, https/tls=tls.connect, socks=stub throws; zero-log, via guardDialing).
- **Utils**: `src/utils/logger.ts` (singleton, zero-dep, reads logLevel/logFile from store), `process-guards.ts` (uncaughtException/unhandledRejection/warning → log only), `cache.ts`/`mq.ts`, `cert.ts` (`loadCerts`/`extractTlsPaths` + `loadTlsContext` two-in-one for http-server/socks/tls)/`ip.ts`/`proxy-helpers.ts` (zero-log, shared `tunnelConnect` on top of `guardDialing`/`bridgeSockets` + `HelperEvent/HelperEventSink` event slot, logging back to server layer via `onEvent` + `guardUpstreamRequest` socket guards, `parseTargetParts` target parser shared by http-pipe/tls, header builders, `isSelfLoop`), `constants.ts` (HTTP response strings, precompiled regex).

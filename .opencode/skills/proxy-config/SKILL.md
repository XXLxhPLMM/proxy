...
**Those three are still the only `config → core` edges, and the discipline above is unchanged.** Do not confuse them with a *fourth* type-only edge that now exists **inside** core: `src/core/types/proxy.ts` type-only imports `ConnectorSource` from `@/core/forward/upstream/connector/index.js` (so `ProxyOptions.connectors` has a type home). That is `types/proxy.ts → forward/upstream/connector/`, i.e. **core → core** — it is *not* a config edge, does not need this exception, and does not weaken the three above. It is listed here only so the next person auditing `import type` hits does not "discover" it and conclude the config exception list has grown. Same erasure rules apply to it (compile-time only, zero runtime edge, no cycle: `connector/types.ts` does not import `types/proxy.ts` at runtime).
...
- **ACL is three layers, pick the right one**:
  ...
  3. **request-time decision** — `src/core/access-control.ts`: the `AccessControl` port's **only** built-in implementation, `createFileAccessControl(config)` → `{ checkClient, checkTarget, checkRoute }` (the three decision functions themselves are module-private and not exported) plus the per-accessor compiled cache.
...
- **There is no second argv entry**: `parseRawArgv` in `sources/argv.ts` is the only argv normaliser and `loadConfig` the only consumer — do not add a second normaliser. Assert argv behaviour through `loadConfig({ argv })`.

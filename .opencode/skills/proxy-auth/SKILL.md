...
- ACL call sites: `src/core/server/admission.ts` (client IP — the single judgment, shared by both inbound paths) reached from `src/core/server/http.ts:handleForward()` + `src/core/server/socks-base.ts:onConn()`, and `src/core/helpers/predial.ts` (target host, after auth / before dial, beside `isSelfLoop`). Order guard: `tests/integration/inbound-admission-order.test.ts`.
